 ...
 */
define( 'DB_Elementor_Form', '2.9.7' );
...
function void_db_elementor_general_admin_notice(){   
            printf(__('<div class="updated notice notice-my-class is-dismissible" data-notice="prefix_deprecated">
                 <p>Need to HIRE Elementor Expert guy? <strong><a href="https://theinnovs.com/hire-elementor-experts" target="_blank"> Hire for Update Issues, Form Issues, CSS Issues, Layout Issues, Cant Edit Issues or Any custom project development.</a></strong></p>
             </div>'));   
}

/**
 * Elementor Activate Notice
 */
function void_db_elementor_activate_notice() {
...
add_action( 'admin_notices', 'void_db_elementor_activate_notice' );

/**
 * CSS For Elementor Activation Notice
 */
function hide_cssfe_noice(){
  $install_date = get_option( 'cssfe_install_time' );
  $past_date = strtotime( '-7 days' );

  if ( $past_date >= $install_date && ! class_exists( 'Css_For_Elementor' )) {
      add_action('admin_notices', 'cssefe_activate_notice');
    }
}
add_action('admin_init', 'hide_cssfe_noice');

function cssefe_activate_notice() {
    $plugin_url = self_admin_url( 'plugin-install.php?s=elements+css+theinnovs&tab=search&type=term' );
    ?>
    <div class="updated notice notice-my-class is-dismissible cssfe_notice">
        <p><?php _e( '<strong></strong> Try <strong><a href=" '. $plugin_url . '">Elementor Extender</a></strong> plugin and get <strong>Elementor Widgets</strong> more EXTENDED. (1K+ active users) <br>Now creating Sticky Header,Hover Effects, Gradients, Custom link in Image Carousel (each image), Sticky Footer & so on is a matter of second!', 'elementoforms' ); ?></p>
    </div>
    <?php
  }
...
function cssefe_include_free_version() {
