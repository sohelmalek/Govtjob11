(function( $ ) {
	...
	// 	// Hook into the "notice-my-class" class we added to the notice, so
	// 	// Only listen to YOUR notices being dismissed
	// 	$( document ).on( 'click', '.cssfe_notice .notice-dismiss', function () {
	// 		// Read the "data-notice" information to track which notice
	// 		// is being dismissed and send it via AJAX
	// 		var type = $( this ).closest( '.cssfe_notice' ).data( 'notice' );
	// 		// Make an AJAX call
	// 		// Since WP 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
	// 		$.ajax( ajaxurl,
	// 		  {
	// 			type: 'POST',
	// 			data: {
	// 			  action: 'dismissed_notice_handler_cssfe',
	// 			  type: type,
	// 			}
	// 		  } );
	// 	  } );
	//   });
	  ...
	  $(document).on('click', '.cssfe_notice .notice-dismiss', function(e) {
		e.preventDefault();
		var id = 'cssfe_install_time';
	
		var data = {
			'action': 'acssfe_maybe_later_will_review',
			'id': id
		};
		// since 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
		jQuery.post(ajax_object.ajax_url, data, function(response) {
			$('.cssfe_notice').fadeOut(1000);
		})
