...
class innovs_Upsell {
    ...
    public function init_hooks() {
        ...
        if ( function_exists( 'innovs_activate' ) && function_exists( 'wpcf7' ) ) {
            return;
        }
        ...
        add_action( 'wp_ajax_innovs_cf7_upsell_installer', array( $this, 'install_innovs' ) );
        add_action( 'wp_ajax_innovs_cf7_installer', array( $this, 'install_innovs_cf7' ) );
    }
    /**
    ...
    public function activation_notice() {
        ...
        <script type="text/javascript">
            jQuery(document).ready( function($) {
                $('#innovs-cf-7-install-now').on('click', function (e) {
                    var self = $(this);
                    e.preventDefault();
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        data: {
                            action: 'innovs_cf7_upsell_installer',
                            _wpnonce: '<?php echo wp_create_nonce('innovs_cf7_upsell_installer'); ?>'
                        },
                ...
                $('#innovs-install-cf7').on('click', function (e) {
                    var self = $(this);
                    e.preventDefault();
                    self.addClass('install-now updating-message');
                    self.text('<?php echo esc_js( 'Installing...' ); ?>');
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        data: {
                            action: 'innovs_cf7_installer',
                            _wpnonce: '<?php echo wp_create_nonce('innovs_cf7_installer'); ?>'
                        },
                        success: function(response) {
                            self.text('<?php echo esc_js( 'Installed' ); ?>');
    ...
    public function install_innovs() {
        check_ajax_referer( 'innovs_cf7_upsell_installer' );
     ...
     * @return void
     */
    public function install_innovs_cf7() {
        check_ajax_referer( 'innovs_cf7_installer' );

        if ( ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( __( 'You don\'t have permission to install the plugins' ) );
        }

        $innovs_status = $this->install_innovs_cf7_plugin( 'contact-form-7', 'wp-contact-form-7.php' );
        $this->fail_on_error( $innovs_status );
     ...
     * @return WP_Error|null
     */
    public function install_innovs_cf7_plugin( $slug, $file ) {
        include_once ABSPATH . 'wp-admin/includes/plugin-install.php';
        include_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
