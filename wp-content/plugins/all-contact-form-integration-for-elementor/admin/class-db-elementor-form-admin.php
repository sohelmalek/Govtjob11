...
class DB_Elementor_Form_Admin {
	...
	public function void_db_element_form_enqueue_styles() {
		...
		//wp_enqueue_style( $this->plugin_name.'bootstrap', plugin_dir_url( __FILE__ ) . 'css/bootstrap.min.css', array(), $this->version );
		wp_enqueue_style( $this->plugin_name.'dataTables', plugin_dir_url( __FILE__ ) . 'css/dataTables.css', array(), $this->version, 'all' );
		//wp_enqueue_style( $this->plugin_name.'contact-form-7', plugin_dir_url( __FILE__ ) . 'css/elementor-contact-form-7.css', array(), $this->version, 'all' );
		//wp_enqueue_style( $this->plugin_name.'gravity-form', plugin_dir_url( __FILE__ ) . 'css/elementor-gravity-form.css', array(), $this->version, 'all' );
		//wp_enqueue_style( $this->plugin_name.'ninja-forms', plugin_dir_url( __FILE__ ) . 'css/elementor-ninja-forms.css', array(), $this->version, 'all' );
	...
	public function void_db_element_form_enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name.'db-elementor', plugin_dir_url( __FILE__ ) . 'js/db-elementor-form-admin.js', array( 'jquery' ), $this->version, true );
		wp_localize_script( $this->plugin_name. 'db-elementor', 'ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' )));
...
public static  function getAllDBFromData (){
	...
	if($wpdb->get_var("SHOW TABLES LIKE '$table'") == $table) {
        ...
        foreach ($datas as $data){
            self::redData($data->id);
        }
        $datass = $wpdb->get_results("SELECT * FROM $table ORDER BY id");
